...
logger = logging.getLogger("membit-agent-v77")
...
if GEMINI_KEY:
    ...
    logger.warning("GEMINI_KEY not set — AI features will be disabled.")
...
async def call_gemini_short(prompt: str, max_tokens: int = 400):
    ...
    if not model:
        return "⚠️ AI Model not initialized.", "AI Model not initialized."
    try:
        ...
        pf = getattr(resp, "prompt_feedback", None)
        if pf and getattr(pf, "block_reason", None):
            return f"⚠️ AI Error: Prompt Blocked ({pf.block_reason})", str(pf.block_reason)
        candidates = getattr(resp, "candidates", None)
        if not candidates:
            return "⚠️ AI Error: No response generated (likely blocked).", "No candidates."
        candidate = candidates[0]
        if getattr(candidate, "finish_reason", "") == 'SAFETY':
            return "⚠️ AI Error: Response was blocked for safety reasons.", "SAFETY_BLOCK"
        text_response = ""
        parts = getattr(candidate, "content", None)
        ...
        if getattr(candidate, "finish_reason", "") == 'MAX_TOKENS':
            text_response += "\n\n⚠️ (AI response was cut short...)"
        if text_response:
            return text_response, ""
    except Exception as e:
        logger.exception("Exception calling Gemini")
        return f"⚠️ AI Error: {e}", str(e)
...
def get_recommendation_from_score(score: int) -> str:
    if score >= 70: return "🛑 High-risk indicators detected. Verify legitimacy before interacting."
    if score >= 40: return "⚠️ Medium risk. Mixed signals — review carefully."
    return "✅ Low/neutral risk indicators. Looks generally safe."
...
@client.event
async def on_ready():
    logger.info(f"Bot ready — model: {MODEL_NAME} | Version: V77 (Clean Build)")
    client.safe_http = safe_http 
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Membit Context • !help"))

# ------------------------
# Command handlers
# ------------------------
async def handle_help(channel):
    embed = discord.Embed(title="Membit Context Agent — Help (V77)", color=0x0099FF)
...
async def handle_analyze(channel, author, text):
    ...
    await channel.send("🧠 Asking AI for sentiment (Safe Mode)...")
    txt, err = await call_gemini_safe(prompt, max_tokens=300) 
    
    if err == "empty" or txt is None:
        txt = "🧠 AI Fallback: The AI could not determine a sentiment. The text might be too neutral or complex."
        err = ""
    if err:
        await channel.send(f"⚠️ AI Error: {err}")
        return
        
    embed = discord.Embed(title="AI Sentiment Analysis", description=truncate(txt, 1500), color=0x00CC99)
    embed.set_footer(text="SAFE-F Mode — AI Minimal, Heuristic Priority")
    await channel.send(embed=embed)
...
async def handle_hunt(channel, author, keyword, short_mode=False):
    ...
    if clusters is None:
        await channel.send("⚠️ Membit API error or network issue.")
        return
    if not clusters:
        await channel.send("❌ No clusters found.")
        return
    ...
    for idx, batch in enumerate(batches):
        ...
        if short_mode:
            embed.set_author(name="LITE MODE • Short results")
        embed.set_footer(text=HACKATHON_FOOTER)
        await channel.send(embed=embed)
        await asyncio.sleep(0.6)

async def handle_trend(channel, keyword):
    await channel.send(f"📈 Gathering trend for {keyword} ...")
    clusters = await fetch_clusters(keyword, limit=8)
    if clusters is None:
        await channel.send("⚠️ Membit API error or network issue.")
        return
    if not clusters:
        await channel.send("❌ No recent clusters found.")
        return
    pos = neg = 0
    for c in clusters:
...
async def handle_context(channel, author, keyword):
    """Versi F — Full Pipeline Aman (AI seminimal mungkin)."""
    await channel.send(f"🌐 SAFE Context Analysis for **{keyword}** ...")
    ...
    if not data or not data.get("clusters"):
        await channel.send("⚠️ Membit API error / no data.")
        return
    ...
    if not ai_insight:
        ai_insight = "AI disabled or quota reached — Heuristic mode active."

    embed = discord.Embed(
        title=f"SAFE Context Report — {keyword}",
...
async def handle_risk(channel, author, text):
    await channel.send("🔬 Scanning for risks (Weighted Engine V2)...")
    ...
    embed.set_footer(text="Weighted Risk Engine V2 — Keyword Scoring Model")
    await channel.send(embed=embed)
...
async def handle_graph(channel, author, keyword):
    await channel.send(f"📊 Building PNG Engagement Graph for **{keyword}** ...")
    data = await safe_http.get_json(CLUSTER_SEARCH_URL, headers=MEMBIT_HEADERS, params={"q": keyword, "limit": 8})
    if not data or "clusters" not in data:
        await channel.send("⚠️ Membit API error or no clusters found.")
        return
    clusters = data.get("clusters", [])
    if not clusters:
        await channel.send("⚠️ No cluster data found to plot.")
        return
...
async def handle_explain(channel, topic):
    ...
    await channel.send(f"📘 Explaining **{topic}** (safe mode)...")
    txt, err = await call_gemini_safe(prompt) 
    if not txt or err:
        txt = f"ℹ️ (Fallback Mode) '{topic}' is a concept related to crypto/web3. No AI available."
    embed = discord.Embed(
        title=f"Explain: {topic}",
        description=truncate(txt, 600),
        color=0x66CCFF
    )
    embed.set_footer(text="SAFE-F Mode — Minimal AI")
    await channel.send(embed=embed)
...
async def on_message(message):
    ...
    if cmd in COMMANDS_WITH_COOLDOWN:
        if within_cooldown(uid):
            await message.channel.send(f"⏳ Wait {COOLDOWN_SECONDS}s.")
            return
        set_cooldown(uid)
    ...
    try:
        ...
        if content == "!ping":
            ...
            await message.channel.send(f"pong — {latency_ms} ms"); return

        if cmd == "!whatis":
            if not args:
                await message.channel.send("⚠️ Usage: `!whatis <term>`"); return
            term = " ".join(args)
            await handle_whatis(message.channel, message.author, term); return
            
        if cmd == "!explain":
            if not args:
                await message.channel.send("⚠️ Usage: `!explain <topic>`")
                return
            topic = " ".join(args)
        ...
        if cmd == "!analyze":
            if not args:
                await message.channel.send("⚠️ Usage: `!analyze <text to analyze>`"); return
            text = " ".join(args)
            await handle_analyze(message.channel, message.author, text); return

        if cmd == "!risk":
            if not args:
                await message.channel.send("⚠️ Usage: `!risk <text to analyze>`"); return
            text = " ".join(args)
            await handle_risk(message.channel, message.author, text); return

        if cmd == "!compare":
            await message.channel.send("🚧 `!compare` is currently under development (MCP Integration Roadmap).")
            return

        if cmd == "!trend":
            if not args:
                await message.channel.send("⚠️ Usage: `!trend <keyword>`"); return
            keyword = " ".join(args)
            await handle_trend(message.channel, keyword); return

        if cmd == "!hunt":
            if not args:
                await message.channel.send("⚠️ Usage: `!hunt <keyword>`"); return
        ...
        if cmd == "!context":
            if not args:
                await message.channel.send("⚠️ Usage: `!context <topic>`")
                return
            topic = " ".join(args)
        ...
        if cmd == "!graph":
            if not args:
                await message.channel.send("⚠️ Usage: `!graph <keyword>`"); return
            keyword = " ".join(args)
            await handle_graph(message.channel, message.author, keyword); return

    except Exception as e:
        logger.exception(f"An error occurred in on_message: {e}")
        await message.channel.send(f"⚠️ An unexpected error occurred. Please tell the admin: `{e}`")
...
if not DISCORD_TOKEN or not MEMBIT_KEY:
    ...
    if GEMINI_KEY and not model:
        logger.warning("Gemini key provided but model failed to initialize — AI features disabled.")
    try:
